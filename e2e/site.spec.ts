...
test('public navigation uses smooth-scroll anchor links', async ({ page }) => {
  ...
  await expect(page.locator('#primary-nav a[href="#news"]')).toHaveAttribute('aria-current', 'location');
});
...
test('manager login remains a separate hidden route', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: /sign in securely/i })).toBeEnabled();
  await expect(page.getByText(/access requires a verified Firebase account/i)).toHaveCount(0);
});
