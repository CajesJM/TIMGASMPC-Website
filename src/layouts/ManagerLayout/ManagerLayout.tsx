...
export function ManagerLayout() {
  ...
  const [collapsed, setCollapsed] = useState(() => window.localStorage.getItem('timgas-manager-sidebar') === 'collapsed');
  const [desktopSidebar, setDesktopSidebar] = useState(() => window.matchMedia('(min-width: 64rem)').matches);
  const [currentHour, setCurrentHour] = useState(() => new Date().getHours());
  const [dashboardSection, setDashboardSection] = useState(() => location.hash.slice(1) || 'overview');
  ...
  useEffect(() => {
    ...
    const firestore = db;
    let active = true;
    void getDoc(doc(firestore, 'adminProfiles', user.uid)).then((snapshot) => {
      if (!active) return;
  ...
  useEffect(() => {
    ...
    const updateActiveSection = () => {
      ...
      const current = sections.reduce<(typeof sections)[number] | undefined>((selected, section) => (
        section.element.getBoundingClientRect().top <= marker ? section : selected
      ), sections[0]);
  ...
  const toggleSidebar = () => {
    setCollapsed((current) => {
      const next = !current;
      window.localStorage.setItem('timgas-manager-sidebar', next ? 'collapsed' : 'expanded');
      return next;
    });
  };

  const navigation = [
    { id: 'overview', label: 'Overview', icon: LayoutDashboard, to: '/manager/preview' },
    { id: 'applications', label: 'Applications', icon: FileText, to: '/manager/preview#applications' },
    { id: 'posts', label: 'Public posts', icon: Megaphone, to: '/manager/posts' },
    { id: 'profile', label: 'Profile', icon: UserRound, to: '/manager/profile' },
  ];
    ...
    <div className={`${styles.shell} ${isSidebarCollapsed ? styles.sidebarCollapsed : ''}`}>
      ...
      <aside className={`${styles.sidebar} ${open ? styles.open : ''}`}>
        ...
        <nav aria-label="Manager navigation">{navigation.map(({ id, label, icon: Icon, to }) => (
          <Link key={id} className={activeSection === id ? styles.active : undefined} to={to} aria-current={activeSection === id ? 'page' : undefined} aria-label={isSidebarCollapsed ? label : undefined} title={isSidebarCollapsed ? label : undefined} onClick={() => { if (id !== 'profile') setDashboardSection(id); setOpen(false); }}><Icon /><span className={styles.sidebarLabel}>{label}</span></Link>
        ))}</nav>
