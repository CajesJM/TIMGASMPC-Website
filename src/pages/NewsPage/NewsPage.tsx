import { ArrowLeft, ArrowRight, Bell, CalendarDays, ChevronDown, ChevronUp, Mail, Newspaper, Phone, Trophy, type LucideIcon } from 'lucide-react';
...
const showcaseGroups: Array<{
...
}> = [
  {
    ...
    title: 'Announcements',
    description: 'Official notices, schedules, advisories, and important member information.',
    icon: Bell,
  },
  {
    category: 'news',
    title: 'Cooperative news',
    description: 'Recent activities, programs, community initiatives, and cooperative updates.',
    icon: Newspaper,
  },
  {
    category: 'achievement',
    title: 'Achievement showcase',
    description: 'Milestones and accomplishments recognized and published by TIMGAS MPC.',
    icon: Trophy,
  },
];
...
export function NewsPage() {
  ...
  useEffect(() => {
    ...
    const observer = typeof ResizeObserver === 'undefined' ? null : new ResizeObserver(measureCarousels);
  ...
  const trackActiveSlide = (category: NewsCategory, event: UIEvent<HTMLDivElement>) => {
    ...
    const nearestIndex = cards.reduce((nearest, card, index) => {
      const nearestDistance = Math.abs(cards[nearest].offsetLeft - carousel.offsetLeft - carousel.scrollLeft);
      const currentDistance = Math.abs(card.offsetLeft - carousel.offsetLeft - carousel.scrollLeft);
      return currentDistance < nearestDistance ? index : nearest;
    }, 0);
    setActiveSlides((current) => current[category] === nearestIndex ? current : { ...current, [category]: nearestIndex });
  };
  ...
  const startMouseDrag = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    dragState.current = { active: true, startX: event.clientX, startScroll: event.currentTarget.scrollLeft };
    event.currentTarget.setPointerCapture(event.pointerId);
  };

  const continueMouseDrag = (event: ReactPointerEvent<HTMLDivElement>) => {
    if (!dragState.current.active) return;
    event.currentTarget.scrollLeft = dragState.current.startScroll - (event.clientX - dragState.current.startX);
  };
          ...
          {loading ? <div className={styles.newsLoading} role="status">Loading official updates…</div> : newsPosts.length > 0 ? <div className={styles.postShowcase}>{showcaseGroups.filter(({ category }) => newsPosts.some((post) => post.category === category)).map(({ category, title, description, icon: Icon }) => {
            ...
            return <section className={styles.postGroup} key={category} aria-labelledby={`${category}-heading`}>
              ...
              <div className={styles.carouselShell}>{overflowingCarousels[category] && <><button className={`${styles.carouselEdgeButton} ${styles.carouselPrevious}`} type="button" onClick={() => moveCarousel(category, -1)} aria-label={`Show previous ${title.toLowerCase()}`}><ArrowLeft /></button><button className={`${styles.carouselEdgeButton} ${styles.carouselNext}`} type="button" onClick={() => moveCarousel(category, 1)} aria-label={`Show next ${title.toLowerCase()}`}><ArrowRight /></button></>}<div className={styles.postGrid} ref={(element) => { carouselRefs.current[category] = element; }} role="region" aria-label={`${title} posts`} tabIndex={0} onScroll={(event) => trackActiveSlide(category, event)} onPointerDown={startMouseDrag} onPointerMove={continueMouseDrag} onPointerUp={stopMouseDrag} onPointerCancel={stopMouseDrag}>{categoryPosts.map((post) => {
                ...
                <div className={`${styles.postPhoto} ${post.photoUrl ? '' : styles.postPhotoPlaceholder}`}>
                  {post.photoUrl ? <img src={post.photoUrl} alt={`Photo for ${post.title}`} loading="lazy" /> : <Icon aria-hidden="true" />}
                </div>
                <div className={styles.postContent}>
                  <div className={styles.postMeta}><time dateTime={post.date}><CalendarDays aria-hidden="true" /> {formatPostDate(post.date)}</time></div>
                  <h4>{post.title}</h4>
                  <p id={descriptionId} className={!isExpanded && hasLongDescription ? styles.postDescriptionCollapsed : undefined}>{post.description}</p>
            ...
            <div>
              ...
              <h2>{loadFailed ? 'Official updates are temporarily unavailable.' : 'No official announcement has been posted yet.'}</h2>
              <p>For current advisories, schedules, program availability, and member notices, contact or visit the TIMGAS MPC office directly.</p>
              <div className={styles.contactLinks}>
                <a href="tel:+639382242376"><Phone size={17} /> +63 938 224 2376</a>
                <a href="mailto:timgascooperative@gmail.com"><Mail size={17} /> timgascooperative@gmail.com</a>
              </div>
            </div>
