...
export function DashboardPage() {
  ...
  return <div className={styles.shell}>
    ...
    <main className={styles.main}>
      <header className={styles.topbar}><button className={styles.mobileMenu} onClick={() => setOpen(true)} aria-label="Open menu"><Menu /></button><div className={styles.search}><Search size={18} /><input aria-label="Search dashboard" placeholder="Search applications" /></div><div className={styles.profile}><button aria-label="Notifications"><Bell /></button><span>{managerInitials}</span><div><strong>{managerEmail}</strong><small>Administrator</small></div><ChevronDown size={16} /></div></header>
      <div className={styles.content}><div className={styles.welcome}><div><p className="eyebrow">Secure manager area</p><h1>Welcome back.</h1><p>Manage the TIMGAS cooperative website and application workflow.</p></div><button>+ New announcement</button></div>
        <div className={styles.demo}><span>Setup phase</span> Authentication is secured with Firebase. Dashboard figures remain interface placeholders until Firestore collections are connected.</div>
