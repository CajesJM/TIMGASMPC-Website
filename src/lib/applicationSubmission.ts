...
export async function submitApplicationWithCaptcha(
  ...
  if (!recaptchaToken) throw new Error("Complete the security check before submitting.");

  const callable = httpsCallable<
    { collection: ApplicationCollection; application: Record<string, unknown>; recaptchaToken: string },
    SubmissionResponse
