...
import styles from "./OfficialLoanReview.module.css";
    ...
    <div className={styles.viewport} aria-label="Official loan application preview">
      <article className={styles.document}>
        ...
        <section className={styles.twoColumnLines}>
          <p><b>Date Filed:</b><span>Recorded upon submission</span></p>
          <p><b>Amount of CBU:</b><span>{formatPeso(data.cbuAmount)}</span></p>
          <p><b>Date Released:</b><span>{data.dateReleased || "Not provided"}</span></p>
          <p><b>Amount of Savings:</b><span>{formatPeso(data.savingsAmount)}</span></p>
        </section>
        ...
        <div className={styles.optionLine}>
          ...
          {loanTypeOptions.map((option) => (
            <span key={option.value} className={data.typeOfLoan === option.value ? styles.checked : undefined}>
              {data.typeOfLoan === option.value ? "✓" : "□"} {option.label}
            </span>
          ))}
        </div>
        <p className={styles.line}><b>Purpose of Loan:</b><span>{data.purposeOfLoan}</span></p>
        <div className={styles.optionLine}>
          <b>Mode of Payment:</b>
          {Object.entries(loanPaymentModeLabels).map(([value, label]) => (
            <span key={value} className={data.paymentMode === value ? styles.checked : undefined}>
              {data.paymentMode === value ? "✓" : "□"} {label}
            </span>
          ))}
        </div>
        <p className={styles.line}><b>Number of months:</b><span>{data.numberOfMonths}</span></p>
        <section className={styles.twoColumnLines}>
          <p><b>Amount Applied:</b><span>{formatPeso(data.amountApplied)}</span></p>
          <p><b>Amount Approved:</b><span>For TIMGAS MPC use</span></p>
        </section>
        ...
        <table className={styles.formTable}>
          ...
          <tbody>
            {data.assets.length > 0 ? Array.from(
        ...
        <table className={styles.formTable}>
          ...
          <tbody>
            {data.debts.length > 0 ? data.debts.map((debt, index) => (
        ...
        <div className={styles.agreementText}>
          <p>I agree and abide by the policies, rules, and regulations governing the credit services of the cooperative. I authorize the cooperative to validate the information above and gather additional information when necessary.</p>
          <p>As co-maker hereof, I acknowledge the restructuring authorization and continuing co-maker obligation stated in the official application form.</p>
        </div>

        <section className={styles.signatureGrid}>
          <div><span>{data.spouseName || "—"}</span><b>With marital consent: Spouse</b></div>
          <div><span>{data.applicantTypedName}</span><b>Applicant/Member Borrower</b></div>
          <div><span>{data.coMakers[0] || "—"}</span><b>CO-MAKER</b></div>
          <div><span>{data.coMakers[1] || "—"}</span><b>CO-MAKER</b></div>
        </section>
      </article>
    </div>
  );
